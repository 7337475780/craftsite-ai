...
export default function ProjectDetailPage() {
  ...
  const loadVersions = useCallback(async () => {
    ...
    try {
      const result = await apiGet(`/api/projects/${id}/versions`);
      if (result.success) {
  ...
  const handleEdit = useCallback(async () => {
    ...
    try {
      const result = await apiPost(`/api/projects/${id}/edit`, {
  ...
  const handleDeleteVersion = useCallback(async (versionId: string) => {
    ...
    try {
      const result = await apiDelete(`/api/projects/${id}/versions/${versionId}`);
      if (!result.success) {
              ...
              <div className="relative rounded-[1.5rem] p-5">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-black/6 pt-4 dark:border-white/6">
                  ...
                  <button
                    ...
                    disabled={isGenerating || isEditing || prompt.trim().length < 5}
