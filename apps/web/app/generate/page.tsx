        ...
        <div className="mx-auto flex w-full max-w-[1600px] items-center justify-between">
          ...
          <div className="hidden items-center gap-2 md:flex">
            ...
            <span className="max-w-[200px] truncate rounded-full border border-violet-400/20 bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-700 dark:border-violet-400/20 dark:bg-violet-500/10 dark:text-violet-300">
              {prompt.trim().length > 0 ? prompt.slice(0, 32) + (prompt.length > 32 ? "…" : "") : "New Generation"}
            </span>
          </div>

          {/* Right: Status + Actions + Theme */}
            ...
            <AnimatePresence>
              {providerInfo && !isGenerating && (
                <motion.div
                  ...
                  className="hidden items-center gap-1.5 rounded-full border border-emerald-400/25 bg-emerald-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wide text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-500/10 dark:text-emerald-300 sm:flex"
                >
                  <Cpu size={11} />
      ...
      <div className="mx-auto flex w-full max-w-[1600px] flex-1 flex-col gap-4 overflow-hidden p-4 lg:flex-row lg:p-5">
        ...
        >
          ...
          <motion.div layout className="flex flex-col gap-1.5">
            ...
            <motion.p
              layout
              className="text-sm text-slate-500 dark:text-white/50"
            >
              {isCompact
          ...
          >
            <div className="relative rounded-[1.5rem] p-5">
              {/* Top inner shine for light mode */}
              ...
              <AnimatePresence mode="popLayout">
                {providerInfo && !isGenerating && (
                  <motion.div
                    ...
                    className="flex items-center gap-2 overflow-hidden rounded-xl border border-emerald-400/25 bg-emerald-50 px-3.5 py-2.5 text-xs font-semibold text-emerald-700 dark:border-emerald-400/20 dark:bg-emerald-500/10 dark:text-emerald-300"
                  >
                ...
                {providerInfo?.isFallback && (
                  <motion.div
                    ...
                    className="mb-4 overflow-hidden rounded-xl border border-orange-400/25 bg-orange-50 px-3.5 py-2.5 text-xs font-semibold text-orange-700 dark:border-orange-400/20 dark:bg-orange-500/10 dark:text-orange-300"
                  >
                    AI providers were busy — showing a safe fallback preview.
                  </motion.div>
                )}
                {error && (
                  <motion.div
              ...
              {/* Quick suggestions — only when empty & not compact */}
              <AnimatePresence>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 border-t border-black/6 pt-4 dark:border-white/6">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    onClick={() =>
                      setWebsiteType(
                        websiteType === "responsive" ? "saas" : "responsive",
                      )
                    }
                    disabled={isGenerating}
                    className={`flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider transition-all duration-200 ${
        ...
        <AnimatePresence mode="popLayout">
          {isCompact && (
            ...
            >
              {/* Canvas navbar */}
              <div className="flex flex-none items-center justify-between px-5 py-3 border-b border-black/6 dark:border-white/6">
                <div className="flex items-center gap-3">
                  {/* Icon */}
